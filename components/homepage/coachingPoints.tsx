...
const CoachingPoints = () => {
	return (
		<div className="p-8 text-gray-700 text-justify col-span-5 lg:col-span-3 flex flex-col gap-8">
			<div className="flex flex-col gap-6 items-center text-center">
				<h3 className="text-4xl font-bold">Is My Coaching Strategy for You?</h3>
				...
				<ul className="flex flex-col gap-2 pl-8">
					{bulletPoints.map((point, index) => (
						<ScrollFade delay={index} start="top 92%" key={index}>
							<li className="text-2xl py-2">{point}</li>
						</ScrollFade>
					))}
				</ul>
				<h3 className="text-4xl font-bold">What You Can Expect</h3>
				<p className="text-xl">
				...
				<h3 className="text-4xl font-bold">How Do I Coach?</h3>
				<ul className="flex flex-col gap-2 pl-8">
					{coachingMethods.map((method, index) => (
						<ScrollFade delay={index} start="top 92%" key={index}>
							<li className="text-2xl py-2">{method}</li>
						</ScrollFade>
					))}
				</ul>
